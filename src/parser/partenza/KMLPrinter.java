...
public class KMLPrinter {
	...
	public static void print(String file, HashMap<Integer, SubPref> map) throws Exception {
		...
		out.println("<name>"+file.substring(0,file.indexOf(".kml"))+"</name>");
		
		/*
		for(SubPref sp : map.values()) {
			out.println("<Placemark>");
			out.println("<name>"+sp.name+"</name>");
			out.println("<Point>");
			out.println("<coordinates>"+sp.lat+","+sp.lon+",0</coordinates>");
			out.println("</Point>");
			out.println("</Placemark>");
		}
		*/
		...
		for(SubPref sp : map.values()) {
			min = Math.min(min, sp.val);
			max = Math.max(max, sp.val);
		}
		
	
		Set<String> scolors = new HashSet<String>();
		for(SubPref sp : map.values()) {
			int index = COLORS.length-1 - (int)( ((double)COLORS.length-1)*(sp.val - min)/(max-min) );
			scolors.add(rgb2kmlstring(COLORS[index]));
		}
		
		for(String c: scolors) {
			out.println("<Style id=\""+c+"\">");
			out.println("<LineStyle>");
			out.println("<color>"+c+"</color>");
			out.println("</LineStyle>");
		...
		for(SubPref sp : map.values()) {		
			out.println("<Placemark>");
			int index = COLORS.length-1 - (int)( (double)(COLORS.length-1)*(sp.val - min)/(max-min) );
			out.println("<styleUrl>#"+rgb2kmlstring(COLORS[index])+"</styleUrl>");
			out.println("<name>"+sp.id+":"+sp.name+"</name>");
			out.println("<description>"+sp.val+"</description>");
			out.println("<Polygon>");
			out.println("<tessellate>1</tessellate>");
			out.println("<outerBoundaryIs>");
			out.println("<LinearRing>");
			out.println("<coordinates>");
			out.println(sp.getKMLRegionCoord()); 
			out.println("</coordinates>");
			out.println("</LinearRing>");
			out.println("</outerBoundaryIs>");
			out.println("</Polygon>");
